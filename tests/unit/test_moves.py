import pytest
from src.president import Move, Card, Rank, Suit, MoveType
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            SINGLE_SM, MoveType.SINGLE, id= "Single"
        ),
        pytest.param(
            DOUBLE_SM, MoveType.DOUBLE, id= "Double"
        ),
        pytest.param(
            TRIPLE_SM, MoveType.TRIPLE, id= "Triple"
        ),
        pytest.param(
            BOMB_SM, MoveType.BOMB, id= "Bomb"
        ),
        pytest.param(
            TRIPLE_SM + DOUBLE_SM, MoveType.FULL_HOUSE, id= "Full house"
        ),
        pytest.param(
            make_sequence(Rank.THREE), MoveType.STRAIGHT, id= "Straight"
        ),
        pytest.param(
            make_sequence(Rank.JACK), None, id= "No straight to Rank.TWO"
        ),
        pytest.param(
        ...
        pytest.param(
            TRIPLE_LG + SINGLE_LG, None, id= "Invalid full house"
        ),
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            SINGLE_LG, SINGLE_SM, True, id= "Larger single"
        ),
        pytest.param(
            DOUBLE_LG, DOUBLE_SM, True, id= "Larger double"
        ),
        pytest.param(
            TRIPLE_LG, TRIPLE_SM, True, id= "Larger triple"
        ),
        pytest.param(
            BOMB_LG, BOMB_SM, False, id= "Bombs flush the pile"
        ),
        pytest.param(
            DOUBLE_SM + TRIPLE_LG, DOUBLE_LG + TRIPLE_SM, True, 
            id= "Larger full house (Larger primary, smaller secondary)"
        ),
        pytest.param(
            DOUBLE_LG + TRIPLE_SM, DOUBLE_SM + TRIPLE_SM, True, 
            id= "Larger full house (Equal primary, larger secondary)"
        ),
        pytest.param(
            make_sequence(Rank.FIVE), make_sequence(Rank.THREE), True, 
            id= "Larger straight"
        ),
        pytest.param(
            DOUBLE_SM, DOUBLE_LG, False, 
            id= "Smaller double"
        ),
        pytest.param(
            DOUBLE_LG + TRIPLE_SM, DOUBLE_SM + TRIPLE_LG, False, 
            id= "Smaller full house (Smaller primary, larger secondary)"
        ),
        pytest.param(
            BOMB_SM, make_sequence(Rank.TEN), True, 
            id= "Bomb anything (but a bomb)"
        ),
        pytest.param(
            make_sequence(Rank.TEN), BOMB_SM, False,
            id="Non-bomb cannot beat bomb"
        ),
        pytest.param(
            TRIPLE_SM + DOUBLE_XL,
            TRIPLE_LG + DOUBLE_LG,
        ...
        pytest.param(
            SINGLE_LG, DOUBLE_SM, False, 
            id= "Different non-bomb move types"
        ),
    ]
)
