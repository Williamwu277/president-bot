...
def get_minimal_card_move(
    possible_moves: list[Move], current_move: Move | None
) -> Move | None:
    ...
    if current_move:
        ...
        sorted_moves = sorted(
            ...
            key=lambda move: (
                len(move),
                max(move.cards[0].rank.value, move.cards[-1].rank.value),
            ),
        )
        ...
        sorted_moves = sorted(
            possible_moves,
            key=lambda move: (
                min(move.cards[0].rank.value, move.cards[-1].rank.value),
                -len(move),
            ),
        )
        return sorted_moves[0]
